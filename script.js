document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===== FUNÇÃO PARA ATUALIZAR ÍCONES DA CENA =====
    function updateSceneIcons(imageText) {
        if (!imageText) {
        ...
        const icons = imageText.split(' ').filter(icon => icon.trim());
        sceneContent.innerHTML = icons.map(icon => 
    ...
    function goBack() {
        if (history.length > 0) {
            ...
            updateGame();
            if (historyBtn && history.length === 0) {
    ...
    if (historyBtn) historyBtn.addEventListener('click', goBack);
